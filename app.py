...
model_id = "MouadNB/fine_tuned_distilhubert"
...
def predict_audio_chunks(audio_path, chunk_duration=30):
    ...
    for i, chunk in enumerate(chunks):
        if len(chunk) < chunk_size:
            # Calculer combien d'échantillons il manque
            missing_samples = chunk_size - len(chunk)
            # Compléter avec le début de l'audio
            padding = audio_array[:missing_samples]
    ...
    with col2:
        ...
        if st.button('Analyze Genre', key='analyze'):
            with st.spinner('Processing audio...'):
                predictions = predict_audio_chunks(uploaded_file)
